                          ...
                          const uint8_t *sk)
{
  ...
  polyveck w1, w0;
  smallpoly cp;
  uint8_t cp_idx[TAU]={0};
  shake256incctx state;

  smallpoly s1_prime[L];
  smallpoly t0[K], s2_prime[K];
...
rej:
  ...
  smallpoly_challenge(&cp, sig);
  smallpoly_getindex(cp_idx,&cp);

  /* Compute z, reject if it reveals secret */
  polyvecl_PSPM(&z, &cp, cp_idx, s1_prime);
  polyvecl_add(&z, &z, &y);
  polyvecl_reduce(&z);
  ...
  {
    poly *tmp = &z.vec[0];
    poly_PSPM_OS(tmp, &cp, cp_idx, &s2_prime[i]);
    poly_reduce(tmp);
    ...
    poly_PSPM_OS(tmp, &cp, cp_idx, &t0[i]);
    poly_reduce(tmp);
                       ...
                       const uint8_t *pk)
{
  ...
  polyvecl mat[K], z;
  polyveck t1, w1, h;
  smallpoly t1_prime[K];
  shake256incctx state;

  if (siglen != CRYPTO_BYTES)
    return -1;

  unpack_pk(rho, t1_prime, pk);
  ...
  smallpoly_challenge(&cp, c);
  polyvec_matrix_expand(mat, rho);
  polyvecl_ntt(&z);
  polyvec_matrix_pointwise_montgomery(&w1, mat, &z);
  polyveck_invntt_tomont(&w1);// w1=Az

  polyveck_PSPM(&t1, &cp, t1_prime);
  polyveck_shiftl(&t1);
